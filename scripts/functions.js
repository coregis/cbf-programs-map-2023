...
function runWhenLoadComplete() {
	if (
		!map.getLayer("raising-school-leaders-points") ||
		!map.getLayer("charles-butt-scholars-points") ||
		!map.getLayer("raising-blended-learners-campuses-points") ||
		!map.getLayer("raising-texas-teachers-points")
	) {
...
function populateZoomControl(
	...
	select.append(new Option(layerName, "-108,25,-88,37,0"));
	...
	for (let i in polygons) {
		...
		if (urlParams["districts"] && urlParams["districts"] === districtType) {
			if (
				urlParams["zoomto"] &&
				decodeURIComponent(urlParams["zoomto"].toString()) ===
					polygons[i].name.toString()
			) {
				zoomToPolygon(
					sourceID,
					polygons[i].bbox.toString() + "," + polygons[i].name,
	...
	for (let i in coords) {
		...
		for (let j in coords[i]) {
			if (!(coords[i][j][0] instanceof Array)) {
				if (coords[i][j][0] < minX) {
					minX = coords[i][j][0];
				}
				if (coords[i][j][0] > maxX) {
					maxX = coords[i][j][0];
				}
				if (coords[i][j][1] < minY) {
					minY = coords[i][j][1];
				}
				...
				for (let k in coords[i][j]) {
					if (coords[i][j][k][0] < minX) {
						minX = coords[i][j][k][0];
					}
					if (coords[i][j][k][0] > maxX) {
						maxX = coords[i][j][k][0];
					}
					if (coords[i][j][k][1] < minY) {
						minY = coords[i][j][k][1];
					}
					if (coords[i][j][k][1] > maxY) {
						maxY = coords[i][j][k][1];
					}
				}
			}
...
function setFilter(sourceID) {
	if (filterStates.year) {
		...
		map.setPaintProperty(sourceID, "circle-stroke-opacity", 1);
		map.setPaintProperty(sourceID, "circle-opacity", 0.3);
	} else {
		console.log(
			"something`s wrong, there should never be no year filter",
			filterStates,
		);
	}
}
...
function moveYearSlider(numberID, increment, loop = false) {
	currentYear = filterStates.year
		? parseInt(filterStates.year, 10)
		: parseInt(slider.value, 10);
	desiredYear = currentYear + increment;

	if (loop) {
		// if we're looping then wrap any overflow around
		if (desiredYear > maxYear) {
			desiredYear = minYear;
		} else if (desiredYear < minYear) {
			desiredYear = maxYear;
		}
	} else {
		// if not looping then keep changes within the min/max bounds
	...
	updateURL(
		(district = filterStates.district ? filterStates.district.val : "0"),
	);
}
...
function zoomToPolygon(sourceID, coords, filterField, maskLayer = true) {
	if (typeof coords !== "undefined") {
		...
		if (sourceID === "esc-regions") {
			showESCRegions = true;
			showHideLayer(
				"state-school-districts-lines",
				(markerNames = ["state_school_districts"]),
				(showOnly = false),
				(hideOnly = true),
			);
		...
		if (maskLayer) {
			...
			if (showHouseDistricts) {
				showHideLayer(
					"state-house-districts-lines",
					(markerNames = ["state_house_districts"]),
					(showOnly = true),
				);
			} else if (showSenateDistricts) {
				showHideLayer(
					"state-senate-districts-lines",
					(markerNames = ["state_senate_districts"]),
					(showOnly = true),
				);
			} else if (showSchoolDistricts) {
				showHideLayer(
		...
		if (maskLayer && filterField !== undefined) {
			setTimeout(function () {
				...
				for (let i in loadedPolygonLayers) {
					...
					if (
						coords[4] !== "0" &&
						loadedPolygonLayers[i][1].replaceAll("_", "-") === sourceID
					) {
						useLayer = true;
				...
				for (let i in loadedPointLayers) {
					...
					if (coords[4] != "0") {
						showHideLayer(
							...
							[loadedPointLayers[i][1], loadedPointLayers[i][1] + "_icon"],
							(showOnly = true),
						);
					}
				}
			...
			map.addLayer({
				...
				paint: {
					"circle-radius": [
						...
						params.circleRadius,
						15,
						params.circleRadius * params.scalingFactor,
						22,
						params.circleRadius * params.scalingFactor * params.scalingFactor,
					],
...
function addVectorLayer(map, params) {
	...
	if (
		params.polygonLayerName !== undefined &&
		params.polygonLayerName !== false
	) {
		if (params.usedInZoomControl) {
